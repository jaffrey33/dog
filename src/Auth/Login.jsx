    ...
    return (
        <main className="min-h-screen bg-home bg-no-repeat bg-cover bg-center bg-fixed flex justify-center items-center pt-24">
            <div className=' w-2/5 bg-slate-200 flex flex-col justify-center items-center p-8 gap-6 rounded-2xl'>
                ...
                <div>OR</div>
                <form onSubmit={onLogin} className='w-4/5 flex flex-col gap-6'>
                    <label htmlFor="email"  >
                        Your Email<br />
                        <input required
                            value={email}
                    ...
                    <button disabled={loader} type='submit'
                        className='w-full px-8 py-4 rounded-full bg-[#111111] hover:bg-black text-white grid place-content-center'>
                        {loader ? <MoonLoader color="white" size={20} /> : "Login"}</button>
                </form>
            </div>
        </main>
    )
